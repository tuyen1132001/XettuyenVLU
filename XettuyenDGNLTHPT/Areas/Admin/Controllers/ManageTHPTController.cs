...
namespace XettuyenDGNLTHPT.Areas.Admin.Controllers
{
    ...
    public class ManageTHPTController : Controller
    {
        ...
        public ActionResult ExportData()
        {
            ...
            Sheet.Cells["AH1"].Value = "Đối tượng ưu tiên";
            Sheet.Cells["AI1"].Value = "Toán";
            Sheet.Cells["AJ1"].Value = "Văn";
            Sheet.Cells["AK1"].Value = "Anh";
            Sheet.Cells["AL1"].Value = "Pháp";
            Sheet.Cells["AM1"].Value = "Lý";
            Sheet.Cells["AN1"].Value = "Hóa";
            Sheet.Cells["AO1"].Value = "Sinh";
            Sheet.Cells["AP1"].Value = "Sử";
            Sheet.Cells["AQ1"].Value = "Địa ";
            Sheet.Cells["AR1"].Value = "GDCD";
            Sheet.Cells["AS1"].Value = "CCNN";
            Sheet.Cells["AT1"].Value = "Mã ngành_tổ hợp 1";
            Sheet.Cells["AU1"].Value = "Tên ngành_tổ hợp 1";
            Sheet.Cells["AV1"].Value = "CTDT 1";
            Sheet.Cells["AW1"].Value = "Mã ngành_tổ hợp 2";
            Sheet.Cells["AX1"].Value = "Tên ngành_tổ hợp 2";
            Sheet.Cells["AY1"].Value = "CTDT 2";
            Sheet.Cells["AZ1"].Value = "Mã ngành_tổ hợp 3";
            Sheet.Cells["BA1"].Value = "Tên ngành_tổ hợp 3";
            Sheet.Cells["BB1"].Value = "CTDT 3";
            Sheet.Cells["BC1"].Value = "Liên lạc địa chỉ";
            Sheet.Cells["BD1"].Value = "Liên lạc mã phường";
            Sheet.Cells["BE1"].Value = "Liên lạc tên phường";
            Sheet.Cells["BF1"].Value = "Liên lạc mã tỉnh tp";
            Sheet.Cells["BG1"].Value = "Liên lạc tên tỉnh tp";
            Sheet.Cells["BH1"].Value = "liên lạc mã quận/huyện";
            Sheet.Cells["BI1"].Value = "Liên lạc tên quận/huyện";
            Sheet.Cells["BJ1"].Value = "Điện thoại di động";
            Sheet.Cells["BK1"].Value = "Điện thoại phụ huynh";
            Sheet.Cells["BL1"].Value = "Data Inserted";
            Sheet.Cells["BM1"].Value = "Data Edited";
            Sheet.Cells["BN1"].Value = "Đã Nhận Hồ sơ";
            Sheet.Cells["BO1"].Value = "Điểm vẽ mỹ thuật";
            Sheet.Cells["BP1"].Value = "Điểm vẽ năng khiếu";
            ...
            foreach (var item in list)
            {
                ...
                Sheet.Cells[string.Format("AH{0}", row)].Value = item.DoiTuongUuTien;
                Sheet.Cells[string.Format("AI{0}", row)].Value = item.Toan;
                Sheet.Cells[string.Format("AJ{0}", row)].Value = item.Van;
                Sheet.Cells[string.Format("AK{0}", row)].Value = item.Anh;
                Sheet.Cells[string.Format("AL{0}", row)].Value = item.Phap;
                Sheet.Cells[string.Format("AM{0}", row)].Value = item.Ly;
                Sheet.Cells[string.Format("AN{0}", row)].Value = item.Hoa;
                Sheet.Cells[string.Format("AO{0}", row)].Value = item.Sinh;
                Sheet.Cells[string.Format("AP{0}", row)].Value = item.Su;
                Sheet.Cells[string.Format("AQ{0}", row)].Value = item.Dia;
                Sheet.Cells[string.Format("AR{0}", row)].Value = item.GDCD;
                Sheet.Cells[string.Format("AS{0}", row)].Value = item.CCNN;
                Sheet.Cells[string.Format("AT{0}", row)].Value = item.MaNganh_ToHop1;
                Sheet.Cells[string.Format("AU{0}", row)].Value = item.TenNganh_TenToHop1;
                Sheet.Cells[string.Format("AV{0}", row)].Value = item.CTDT1;
                Sheet.Cells[string.Format("AW{0}", row)].Value = item.MaNganh_ToHop2;
                Sheet.Cells[string.Format("AX{0}", row)].Value = item.TenNganh_TenToHop2;
                Sheet.Cells[string.Format("AY{0}", row)].Value = item.CTDT2;
                Sheet.Cells[string.Format("AZ{0}", row)].Value = item.MaNganh_ToHop3;
                Sheet.Cells[string.Format("BA{0}", row)].Value = item.TenNganh_TenToHop3;
                Sheet.Cells[string.Format("BB{0}", row)].Value = item.CTDT3;
                Sheet.Cells[string.Format("BC{0}", row)].Value = item.LienLac_DiaChi;
                Sheet.Cells[string.Format("BD{0}", row)].Value = item.LienLac_MaPhuongXa;
                Sheet.Cells[string.Format("BE{0}", row)].Value = item.LienLac_TenPhuongXa;
                Sheet.Cells[string.Format("BF{0}", row)].Value = item.LienLac_MaTP;
                Sheet.Cells[string.Format("BG{0}", row)].Value = item.LienLac_TenTP;
                Sheet.Cells[string.Format("BH{0}", row)].Value = item.LienLac_MaQH;
                Sheet.Cells[string.Format("BI{0}", row)].Value = item.LienLac_TenQH;
                Sheet.Cells[string.Format("BJ{0}", row)].Value = item.DienThoaiDD;
                Sheet.Cells[string.Format("BK{0}", row)].Value = item.DienThoaiPhuHuynh;
                Sheet.Cells[string.Format("BL{0}", row)].Value = Convert.ToDateTime(item.DateInserted).ToString("dd/MM/yyyy");
                Sheet.Cells[string.Format("BM{0}", row)].Value = Convert.ToDateTime(item.DateEdited).ToString("dd/MM/yyyy");
                Sheet.Cells[string.Format("BN{0}", row)].Value = item.DaNhanHoSo;
                Sheet.Cells[string.Format("BO{0}", row)].Value = item.DiemVeMyThuat;
                Sheet.Cells[string.Format("BP{0}", row)].Value = item.DiemVeNangKhieu;
                row++;
            }
